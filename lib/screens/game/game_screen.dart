...
import 'package:tigers_journey_evolution/screens/tutor_screen.dart';
  ...
  void onShowFinalDialog(BuildContext context, GameProvider provider) async {
    ...
    if (answer.correct == Correctness.correct) {
      ...
      if (res == 2) {
        ...
        provider.premium) {
      final wrong = answer.correct == Correctness.wrong && !provider.premium;
      final res = await showWarningDialog(context, wrong);
      if (res) {
        provider.onRestart();
      } else {
        provider.onMenu();
